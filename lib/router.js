...
FlowRouter.route('/a', {
  action(params) {
    document.title = 'Tekst & inhoud | Wij Nieuwe Westen';
...
FlowRouter.route('/login', {
  async action(params) {
    const { default: Login } = await import('../imports/ui/components/Login/Login.jsx')
    document.title = 'Login | Wij Nieuwe Westen';
    mount(AppProvider, { content: <Login /> });
  }
})
FlowRouter.route('/login/:token', {
  async action(params) {
    // Login now
    window.defaultSuccessHandler(params.token, 'login')
    mount(AppProvider, { content: <Landing /> });
  }
})
